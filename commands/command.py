...
class CmdOpen(default_cmds.CmdOpen):
    ...
    def create_exit(self, exit_name, location, destination, exit_aliases=None, typeclass=None):
        new_exit = super().create_exit(exit_name, location, destination, exit_aliases=exit_aliases, typeclass=typeclass)
        ...
        if inherits_from(new_exit, Door):
            ...
            back_exit = self.create_exit(exit_name, destination, location, exit_aliases=exit_aliases, typeclass=typeclass)
...
class CmdDrop(default_cmds.CmdDrop):
    __doc__ = default_cmds.CmdDrop.__doc__
    ...
    def func(self):
        caller = self.caller
        if not self.args:
            caller.msg(f"|yWhat are you trying to drop?")
            return
        args = self.args.lstrip()
        try:
            objs = caller.search(args, quiet=True)
            obj = objs[0]
        except IndexError:
            caller.msg(f"|yWhat item are you referring to?")
            return
        if caller == obj:
            caller.msg(f"|yYou cannot drop yourself!")
            return
        if not obj.location == caller:
            caller.msg(f"|yThe %s is not in your inventory!" % obj)
            return
        if obj.db.undroppable == True:
            caller.msg(f"|yYou cannot drop the %s!" % obj)
            return
        obj.move_to(caller.location, quiet=True)
        obj.at_drop(caller)
        message = "%s drop%s the %s."
